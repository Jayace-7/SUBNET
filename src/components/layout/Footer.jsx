  ...
  return (
    <footer className="mt-16 border-t border-white/10 bg-[#080b0b]">
      <div className="mx-auto max-w-7xl px-6 py-12 sm:px-8 lg:px-10 lg:py-16">
        <div className="flex flex-col gap-10 border-b border-white/10 pb-10 lg:flex-row lg:items-start lg:justify-between lg:gap-16">
          <div className="max-w-sm">
            ...
            <a
              ...
              className="mt-5 inline-flex items-center gap-2 text-sm font-medium text-white/75 transition-colors hover:text-[#00D4C7]"
            >
              <FiMail size={16} />
              support@subnet.com
            </a>
          ...
          <div className="flex items-center gap-3">
            {SOCIAL_LINKS.map(({ label, icon: Icon }) => (
              <a
                ...
                href="#"
                aria-label={label}
                className="flex h-9 w-9 items-center justify-center rounded-full border border-white/15 text-white/65 transition-all duration-200 hover:border-[#00D4C7]/70 hover:bg-[#00D4C7]/10 hover:text-[#00D4C7]"
              >
                <Icon size={16} />
              </a>
            ))}
        ...
        <div className="mt-7 flex flex-col gap-2 text-xs text-white/35 sm:flex-row sm:items-center sm:justify-between">
          <p>© {new Date().getFullYear()} SUBNET. All rights reserved.</p>
          <p>Made for stories that stay with you.</p>
        </div>
      </div>
    </footer>
  );
